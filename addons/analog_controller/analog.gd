...
export(bool) var isDynamicallyShowing = true #false
...
####__Changed
#signal stop_contoller
var stop_contoller = false
func stop_cont(stop = false):
	stop_contoller = stop
#		return !stop
#emit_signal("stop_contoller")
...
func _input(event):
	...
	if need2ChangeActivePointer(event):
		...
		if (currentPointerIDX != incomingPointer) and event.is_pressed() and !stop_contoller: #changed
...
func need2ChangeActivePointer(event):
	...
	if event is InputEventMouseMotion or event is InputEventMouseButton and true:
